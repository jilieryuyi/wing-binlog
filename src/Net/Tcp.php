...
class Tcp
{
    ...
    public function debug()
    {
        if ($this->debug) {
            ...
            wing_debug("请求次数/失败次数/发送失败/每秒处理 ==> " . $this->accept_times . "/" . $this->error_times . "/" . $this->send_fail_times . "/" . $s );
            wing_debug("当前连接数", count($this->clients), "-buffers数量", count($this->buffers));
        }
    }
