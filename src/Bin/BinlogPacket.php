...
class BinLogPacket
{
	public static function parse($pack, $checksum)
	{
        //package error
        if (strlen($pack) < 19) {
...
//        $this->advance(1);
        ...
        //self::$EVENT_INFO['time'] =
		//事件发生的时间
		$timestamp  = unpack('L', $packet->read(4))[1];
        //self::$EVENT_INFO['type'] = self::$EVENT_TYPE
		$event_type = unpack('C', $packet->read(1))[1];
        //self::$EVENT_INFO['id']   =
		$server_id  = unpack('L', $packet->read(4))[1];
        //self::$EVENT_INFO['size'] =
		$event_size = unpack('L', $packet->read(4))[1];

        //position of the next event
        //self::$EVENT_INFO['pos']  =
		$log_pos    = unpack('L', $packet->read(4))[1];//
        //self::$EVENT_INFO['flag'] =
		$flags      = unpack('S', $packet->read(2))[1];
		...
		return [$data, $binlog_file, $log_pos];
